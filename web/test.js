...
function _clearDisplay(r,g,b){ ctx.fillStyle = "rgb(" + r + ", " + g + ", " + b + ")"; ctx.fillRect(0, 0, canvas.width, canvas.height); }
function _drawObject(x,y,width,height,xResolution,yResolution,id,cacheAllowed,pixels){ if (cacheAllowed && imageCache[id] != undefined) { ctx.drawImage(imageCache[id], x, y, width, height); return; } let size = width * height * 4; let pixelArray = new Uint8ClampedArray(Module.HEAPU8.buffer, pixels, size); let imageData = new ImageData(pixelArray, width, height); createImageBitmap(imageData).then(image => { if (cacheAllowed) imageCache[id] = image; ctx.drawImage(image, x, y, width, height); }); }
function _startIO(screenWidth,screenHeight,fps){ canvas = document.querySelector("#canvas"); canvas.width = screenWidth; canvas.height = screenHeight; ctx = canvas.getContext("2d", { alpha: true }); imageCache = {}; }
function _updateIO(){ }
